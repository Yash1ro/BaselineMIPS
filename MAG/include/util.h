...
namespace MAG {
    ...
    inline float* data_align(float* data_ori, unsigned point_num, unsigned& dim){
      ...
      const size_t total = (size_t)point_num * (size_t)new_dim;
      //std::cout << "align to new dim: "<<new_dim << std::endl;
      #ifdef __APPLE__
        data_new = new float[total];
      #else
        data_new = (float*)memalign(DATA_ALIGN_FACTOR * 4, total * sizeof(float));
        if (data_new == nullptr) {
          std::cerr << "memalign failed in data_align" << std::endl;
          exit(-1);
        }
      #endif
