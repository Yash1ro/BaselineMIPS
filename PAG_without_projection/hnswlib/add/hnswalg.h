                    ...
                    for (size_t j = 0; j < size; j++) {
                        ...
                        if (!(visited_array[candidate_id] == visited_array_tag)) {
							...
							if(retset[retset_size-1].flag == true){
								...
								top_candidates.emplace(retset[retset_size-1].distance, retset[retset_size-1].id);
							}
...
size2 = w;
        ...
        void searchKnn(float* query_point, float* query_extended_point, float* query_extended_point0, size_t K, std::vector<Neighbor>& final_result, float* table, int step, std::vector<Node>& InitialTable) const{
            ...
            for(int i = 0; i < m0_; i++){ 
				...
				if(cur_val < 0){
					cur_id += m0_;
					cur_val = -1.0f*cur_val;
				}		
				if(cur_val > max_ip){
					max_ip = cur_val;
					max_id1 = cur_id;
				}
				cur_pos0 += extended_dim_/2;
            }

			max_ip = -1.0f;
            y = query_extended_point0 + extended_dim_/2;
            for(int i = 0; i < m0_; i++){ 
                float cur_val = fstsubfunc0_((void*) y, (void*) cur_pos0, sub_func_param0_);                
				int cur_id = i;
				if(cur_val < 0){
...
//	    max_id1 = 0; max_id2=0;
			...
			Node* InitialNode = &InitialTable[ (size_t)(max_id1* 2 * m0_+max_id2) * maxk_];
			...
			for(int i = 0; i < step; i++){
				...
				float cur_ip = dot_product_avx512_int16(query_point_int16, (int16_t*) norm_pointer, qscale, vscale);
                ...
                retset[i] = Neighbor(*(int*)(norm_pointer+(padded_dim_/2)), cur_dist, cur_ip, true, index_pointer);
...
//sum = _mm512_fmadd_ps(sum, v_mul, v_add);
        ...
        void saveIndex() {
            ...
            for (int i = 0; i < 2; ++i) {
                for (int j = 0; j < m0_; ++j) {
                    output.write(reinterpret_cast<char*>(projVec0_[i][j].data()), extended_dim_ * sizeof(float)/2);
                }
            }
        ...
        void loadIndex(const char* path_index, SpaceInterface<dist_t> *s1, SpaceInterface<dist_t> *s2, SpaceInterface<dist_t> *s3, SpaceInterface<dist_t> *s4, size_t max_elements_i=0) {
            ...
            for (size_t i = 0; i < 2; ++i) {
                for (size_t j = 0; j < m0_; ++j) {	
                    input.read(reinterpret_cast<char*>(cur_pos), extended_dim_ * sizeof(float) / 2);
                    cur_pos += extended_dim_/2;
				}
            }
