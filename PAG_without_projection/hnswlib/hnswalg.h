...
namespace hnswlib {
    ...
    public:
        ...
        HierarchicalNSW(SpaceInterface<dist_t> *s1, SpaceInterface<dist_t> *s2, SpaceInterface<dist_t> *s3, SpaceInterface<dist_t> *s4, const char* path_index, bool nmslib = false, size_t max_elements=0) {
			loadIndex(path_index, s1, s2, s3, s4, max_elements);		
        }

        HierarchicalNSW(size_t maxk, int step, size_t m, size_t vecdim, std::vector<std::vector<std::vector<float>>>& projVec, int level, int subdim, std::vector<int>& permutation, SpaceInterface<dist_t> *s1, SpaceInterface<dist_t> *s2, SpaceInterface<dist_t> *s3, SpaceInterface<dist_t> *s4, 
		        size_t max_elements, char* path_index, size_t m0, std::vector<std::vector<std::vector<float>>>& projVec0, std::vector<int>& permutation0, size_t M = 16, size_t ef_construction = 200) :
			...
			fstsubfunc0_ = s4->get_dist_func();
			sub_func_param0_ = s4->get_dist_func_param();
			...
			segment_size_ = 16 * (half_level + 2 * sizeof(int16_t)); // 3->2
            //segment_size_new_ = 16 * (half_level + 2 * sizeof(int16_t)); // 3->2

			size_links_level0_ = maxM0_ * (2 * sizeof(int16_t) + sizeof(tableint) + half_level) + sizeof(linklistsizeint);   //new 3->2
            ...
            sec_part_ = data_size_ + 2 * sizeof(float) + sizeof(labeltype);
            
            init_size_ = maxM0_ * sizeof(tableint) + segment_size_ + sizeof(linklistsizeint);

          //  init_size_ = maxM0_ * sizeof(tableint) + 4*segment_size_ + sizeof(linklistsizeint);
        ...
        ~HierarchicalNSW() {
			...
			if(projVecQuery0 != NULL){
				delete[] projVecQuery0;
			}			
        }
        ...
        std::vector<std::vector<std::vector<float>>> projVec_;
        ...
        std::vector<int> permutation0_;
		int m0_;
		std::vector<std::vector<std::vector<float>>> projVec0_;
		...
		DISTFUNC<dist_t> fstsubfunc0_;
		
        void *dist_func_param_;
		void *sub_func_param_;
		void *sub_func_param0_;
...
/*
        inline uint16_t* getNeighborNorm(char* data, int i) const {
			int a = i / 16;
			int b = i % 16; 			
			char* data_offset = data + a * segment_size_ + offset3 + b * sizeof(int16_t);
			return (uint16_t* )data_offset;
        }	
*/
        ...
        searchBaseLayerFast(tableint cur_id, tableint ep_id, const void *query_point){
            ...
            for(int j = 0; j < level_; j++){
                ...
                for(int i = 0; i < m_; i++){  
                    _mm_prefetch(reinterpret_cast<const char*>(projVec_[j][i].data()), _MM_HINT_T0);
                    current_level_ptr[i] = -1 * fstsubfunc_((void*) y, (void*) projVec_[j][i].data(), sub_func_param_);
                ...
                if (retset[k].flag) {
					...
					if(l_num < LL){	
						...
						if(reverse_flag == true){
							insert_neighbor(addNeighbors, n, cur_id, cur_dist);
						}
					}
					else{
...
//sum = _mm512_sub_ps(sum, v_val);

sum = _mm512_fmsub_ps(sum, v_mul, v_val);
_mm512_store_ps(Thres, sum);
						    ...
						    for(int i = 0; i < check_num; i++){
							    if(Thres[i] <= 0){
								    ...
								    count++;
							        if(reverse_flag == true && Thres[i] <= difference_)
								        reverse_flag = false;															
							    }
						    }
			...
			for(int i = 0; i < l_num; i++){
			    top_candidates.emplace( retset[i].distance, retset[i].id);
			}
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        searchBaseLayerFast2(tableint cur_id, const void *query_point_tmp){

            int step = step_;
					
			std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;

			float qnorm = fstipfunc_(query_point_tmp, query_point_tmp, dist_func_param_)/2;		

            float true_qnorm = sqrt(2*qnorm);

			float qnorm_vec[padded_dim_];
			float* cur_query_point = (float*) query_point_tmp;
			for(int i = 0; i < padded_dim_; i++)
				qnorm_vec[i] = cur_query_point[i]/true_qnorm;
			
			float q_scale = 32767.0f / find_max_abs(qnorm_vec);
			
			int16_t query_point_int16[padded_dim_];
			for(int i = 0; i < padded_dim_; i++)
				query_point_int16[i] = float_to_int16(qnorm_vec[i], q_scale);	
			
            float* query_extended_point = new float[extended_dim_];
			permute(query_extended_point, qnorm_vec);
            ...
            std::vector<NeighborIndex> retset2_buf(LL2);
...
_mm512_store_ps(Thres, sum);
					...
					if(reverse_flag == true){
						insert_neighbor(addNeighbors, n, cur_id, true_qnorm*cur_dist);
					}
                    ...
                    for (size_t j = 0; j < size; j++) {
                        ...
                        if (!(visited_array[candidate_id] == visited_array_tag)) {
							...
							if(retset[retset_size-1].flag == true){
								...
								top_candidates.emplace(true_qnorm * retset[retset_size-1].distance, retset[retset_size-1].id);
							}
			    ...
			    for(int i = 0; i < retset_size; i++){
			        top_candidates.emplace(true_qnorm*retset[i].distance, retset[i].id);
			    } 		
...
while (w < LL2 && p2 >= ii && p3 >= jj) {
    if (retset2[p2].distance > retset3[p3].distance) {
        retset2_buf[w++] = retset2[p2--];
    } else {
        retset2_buf[w++] = retset3[p3--];
    }
}
...
size2 = w;
        ...
        void searchKnn(float* query_point, float* query_extended_point, float* query_extended_point0, size_t K, std::vector<Neighbor>& final_result, float* table, int step, std::vector<Node>& InitialTable) const{
			...
			int16_t query_point_int16[padded_dim_];
			for(int i = 0; i < padded_dim_; i++)
				query_point_int16[i] = float_to_int16(qnorm_vec[i], qscale);
			...
			std::vector<Neighbor> retset2_buf(LL2);			
            ...
            //----------------new code-----------------------
/*
const int signed_m = 2 * m0_;
const int pair_cols = signed_m * signed_m;
const int subdim = extended_dim_ / 4;

std::vector<float> raw_proj(4 * m0_);
std::vector<float> pair01(pair_cols);
std::vector<float> pair23(pair_cols);

float* cur_pos0 = projVecQuery0;
...
// ------------------------------------------------------------
float max_ip = -1.0f;
float* y = query_extended_point0;
for (int i = 0; i < m0_; i++) {
    float cur_val = fstsubfunc0_((void*)y, (void*)cur_pos0, sub_func_param0_);
    raw_proj[i] = cur_val;

    int cur_id = i;
    float abs_val = cur_val;
    if (abs_val < 0.0f) {
        cur_id += m0_;
        abs_val = -abs_val;
    }
...
// ------------------------------------------------------------
max_ip = -1.0f;
y = query_extended_point0 + subdim;
for (int i = 0; i < m0_; i++) {
    float cur_val = fstsubfunc0_((void*)y, (void*)cur_pos0, sub_func_param0_);
    raw_proj[m0_ + i] = cur_val;
...
// ------------------------------------------------------------
max_ip = -1.0f;
y = query_extended_point0 + 2 * subdim;
for (int i = 0; i < m0_; i++) {
    float cur_val = fstsubfunc0_((void*)y, (void*)cur_pos0, sub_func_param0_);
    raw_proj[2 * m0_ + i] = cur_val;
...
// ------------------------------------------------------------
max_ip = -1.0f;
y = query_extended_point0 + 3 * subdim;
for (int i = 0; i < m0_; i++) {
    float cur_val = fstsubfunc0_((void*)y, (void*)cur_pos0, sub_func_param0_);
    raw_proj[3 * m0_ + i] = cur_val;
...
// ------------------------------------------------------------
// 用 4 组 raw_proj 构造 signed 投影后的两个 pair 空间
// pair01[(a,b)] = p0[a] + p1[b]
// pair23[(c,d)] = p2[c] + p3[d]
// 其中 a,b,c,d ∈ [0, 2*m0_)
// ------------------------------------------------------------
for (int a = 0; a < signed_m; ++a) {
    const float va = (a < m0_) ? raw_proj[a] : -raw_proj[a - m0_];
    for (int b = 0; b < signed_m; ++b) {
        const float vb = (b < m0_) ? raw_proj[m0_ + b] : -raw_proj[m0_ + (b - m0_)];
        pair01[a * signed_m + b] = va + vb;
    }
}

for (int c = 0; c < signed_m; ++c) {
    const float vc = (c < m0_) ? raw_proj[2 * m0_ + c] : -raw_proj[2 * m0_ + (c - m0_)];
    for (int d = 0; d < signed_m; ++d) {
        const float vd = (d < m0_) ? raw_proj[3 * m0_ + d] : -raw_proj[3 * m0_ + (d - m0_)];
        pair23[c * signed_m + d] = vc + vd;
    }
}
...
// 用最优 signed id 找进入列
// ------------------------------------------------------------
const int pair01_id = max_id1 * signed_m + max_id2;
const int pair23_id = max_id3 * signed_m + max_id4;
const size_t col_id =
    static_cast<size_t>(pair01_id) * pair_cols + static_cast<size_t>(pair23_id);

Node* InitialNode = InitialTable.data() + col_id * static_cast<size_t>(maxk_);

//InitialNode = InitialTable.data();
// ------------------------------------------------------------
// 先取进入点（这里还是按你原来的逻辑）
// ------------------------------------------------------------
int next_id = InitialNode[0].index;
*/
int next_id = 0;
for (int i = 0; i < step; i++) {
    int cur_id = next_id;
   ...
   // int cur_id = i;
    ...
    if (i != step - 1) {
        //next_id = InitialNode[i + 1].index;
		next_id = i + 1;
        _mm_prefetch((char*)(visited_array + next_id), _MM_HINT_T0);
    }

    float* norm_pointer = (float*)getNormByInternalIdQuery(cur_id);
    ...
    float cur_ip = dot_product_avx512_int16(
        query_point_int16,
        (int16_t*)norm_pointer,
        qscale,
        vscale);

    float cur_dist = -1.0f * true_norm * cur_ip;

    cur_round = max_round - 1;
    for (int r = 1; r < max_round; r++) {
        if (cur_id < num_edge_offset[r]) {
            cur_round = r - 1;
            break;
        }
    }
    ...
    index_pointer =
        (data_level0_memory_query_ + size_links_offset_[cur_round] +
         (cur_id - num_edge_offset[cur_round]) * size_data_per_element_new_[cur_round]);

    retset[i] = Neighbor(
        *(int*)(norm_pointer + (padded_dim_ / 2)),
        cur_dist,
        cur_ip,
        true,
        index_pointer);
...
//sum = _mm512_fmadd_ps(sum, v_mul, v_add);
        ...
        inline void permute0(float* dst, float* src){
			for(int i = 0; i < extended_dim_; i++){
				int new_pos = permutation0_[i];
                if(new_pos < padded_dim_)
				    dst[i] = src[new_pos];
				else
					dst[i] = 0.0f;				
						...
						for(int i = 0; i < sz_link_list_other; i++){
                            ...
                            for(int j = 0; j < indx; j++){
								if(cur_id == arr[j]){
		                            ...
		                            //  if(flag==true) {assert(cur_pos < (int)maxM0_);}				
						if(flag == true){
					        data[cur_pos] = cur_c;
        ...
        void saveIndex() {
            ...
            for (int i = 0; i < level_; ++i) {
                for (int j = 0; j < m_; ++j) {
                    output.write(reinterpret_cast<char*>(projVec_[i][j].data()), subdim_ * sizeof(float));
                }
            }

            for (int i = 0; i < 4; ++i) {
                for (int j = 0; j < m0_; ++j) {
                    output.write(reinterpret_cast<char*>(projVec0_[i][j].data()), extended_dim_ * sizeof(float)/4);
                }
            }
        ...
        void loadIndex(const char* path_index, SpaceInterface<dist_t> *s1, SpaceInterface<dist_t> *s2, SpaceInterface<dist_t> *s3, SpaceInterface<dist_t> *s4, size_t max_elements_i=0) {
            std::string folderPath(path_index);
            std::string fullPath;
            if (!folderPath.empty() && (folderPath.back() == '/' || folderPath.back() == '\\')) {
                indexPath_ = folderPath + "index.bin";
			...
			projVecQuery0 = new float[m0_* extended_dim_];
            ...
            cur_pos = projVecQuery0;
            for (size_t i = 0; i < 4; ++i) {
                for (size_t j = 0; j < m0_; ++j) {	
                    input.read(reinterpret_cast<char*>(cur_pos), extended_dim_ * sizeof(float) / 4);
                    cur_pos += extended_dim_/4;
				}
            }
        ...
        float calc_vec_adjust(float* obj_vec, float* cen_vec, float* diff_vec, float obj_norm, float cen_norm){		
			...
			float cos0 = (float) (dot_product_avx512_extended(obj_vec, tmp_cen) / obj_norm);
			float cos = (float) (cos0 / cen_norm);
        ...
        inline float calc_dist_int16(int id1, int id2) const{
			...
			float dist = -1.0f*norm1*norm2*dot_product_avx512_int16((int16_t*)norm_pointer1,(int16_t*)norm_pointer2,scale1,scale2);							
        ...
        inline float calc_dist_float_int16(float* v, int id) const{			
            ...
            float dist = -1.0f*norm*dot_product_avx512_f32_i16(v, (int16_t*)norm_pointer, scale);
			return dist;
		}
        ...
        void addProjVal(int id, int pos, float dist){
			...
			float tmp_adjust = calc_vec_adjust(extended_obj, extended_center, diff_data, obj_norm, center_norm);
			...
			for(int k = 0; k < level_; k++){
		        for(int j = 0; j < m_; j++){
			        ...
			        for(int l = 0; l < subdim_; l++){
			            sum += LSH_data[k * subdim_ + l] * projVec_[k][j][l];
			        }
			...
			for(int k = 0; k < level_; k++){
			    for(int l = 0; l < subdim_; l++){
			        ...
			        cen_vec[k * subdim_ + l] =  projVec_[k][s][l] * s1;
			    }					
		    }
			...
			float tmp_norm2 = dot_product_avx512_extended(LSH_data, LSH_data);	
                ...
                float stored_val2 = (tmp_last - (error_val * tmp_adjust / tmp_norm2))/center_norm;
        ...
        void addPoint(const void *data_point, labeltype label, float* norm) {//warning
			...
			*scale_pointer = 32767.0f / find_max_abs(vec_normalized);			
			norm_pointer += 4;
			int16_t* dst_vec = (int16_t*) norm_pointer;

			for(int j = 0; j < padded_dim_; j++)
                dst_vec[j] = float_to_int16( vec_normalized[j], *scale_pointer);
        ...
        void mapping(){
            mapTable.resize(vecsize_);
#pragma omp parallel for schedule(dynamic)			
		    for(int i = 0; i < vecsize_; i++){
			    unsigned external_id = *getExternalLabeLp(i);
	            mapTable[external_id] = i;	    
			}			
		}


/*
void buildTable(std::vector<std::vector<float>>& projVal_, std::vector<std::vector<float>> & vec0_, std::vector<Node>& InitialTable) {

    #pragma omp parallel
    {
        std::vector<float> tmp(extended_dim_);

        #pragma omp for schedule(static)
        for (int i = 0; i < vecsize_; i++) {
            permute0(tmp.data(), vec0_[i].data());
            std::memcpy(vec0_[i].data(), tmp.data(), extended_dim_ * sizeof(float));
        }
    }
    ...
    for (int i = 0; i < vecsize_; i++) {
        for (int j = 0; j < 2; j++) {
            for (int l = 0; l < m0_; l++) {
                float val = fstsubfunc0_(vec0_[i].data() + (j * extended_dim_ / 2), projVec0_[j][l].data(), sub_func_param0_);
                projVal_[i][2*j*m0_+l] = val;
                projVal_[i][2*j*m0_+m0_+l] = -val;
            }
        }
    }

    const int large_m = 4 * m0_ * m0_;

    //std::vector<Node> InitialTable;
    //InitialTable.resize(static_cast<size_t>(large_m) * maxk_);

    std::atomic<int> completed_tasks(0);
    int progress_step = large_m / 100;
    if (progress_step == 0) progress_step = 1;

    #pragma omp parallel
    {
        std::vector<Node> sorted_array(vecsize_);

        #pragma omp for schedule(static)
        for (int j = 0; j < large_m; j++) {

            int j1 = j / (2*m0_);
            int j2 = j % (2*m0_);

            for (int l = 0; l < vecsize_; l++) {
                sorted_array[l].value = projVal_[l][j1] + projVal_[l][2*m0_+j2];
                sorted_array[l].index = mapTable[l];
            }

            std::nth_element(sorted_array.begin(), 
                sorted_array.begin() + maxk_, 
                sorted_array.end(), 
                [](const Node& a, const Node& b) {
                return a.value > b.value; 
            });

            Node* out = InitialTable.data() + static_cast<size_t>(j) * maxk_;
            for (int l = 0; l < maxk_; l++) {
                out[l] = sorted_array[l];
            }
        }
		
        int local_completed = ++completed_tasks;
        if (local_completed % progress_step == 0 || local_completed == large_m) {
            #pragma omp critical
            {
                float percentage = (float)local_completed / large_m * 100.0f;
                std::cout << "\rProgress: " << percentage << "% (" 
                          << local_completed << "/" << large_m << ")" << std::flush;
            }
        }		
    }
}
...
/*
void buildTable(std::vector<std::vector<float>>& projVal_, std::vector<std::vector<float>> & vec0_, std::vector<Node>& InitialTable) {
    const int num_cols = 4 * m0_; 

    // 1. 连续化向量（保持你的逻辑，但合并并行块减少开销）
    #pragma omp parallel
    {
        std::vector<float> tmp(extended_dim_);
        #pragma omp for schedule(static)
    ...
    // 2. 计算投影值并存入连续的大块内存（关键：为了转置做准备）
    // 建议：直接用一维数组替代 vector<vector>，提高缓存友好度
    std::vector<float> projVal_flat(static_cast<size_t>(vecsize_) * num_cols);
    
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < vecsize_; i++) {
        float* row_ptr = projVal_flat.data() + static_cast<size_t>(i) * num_cols;
        for (int j = 0; j < 2; j++) {
            float* vec_data = vec0_[i].data() + (j * extended_dim_ / 2);
            for (int l = 0; l < m0_; l++) {
                float val = fstsubfunc0_(vec_data, projVec0_[j][l].data(), sub_func_param0_);
                row_ptr[2*j*m0_+l] = val;
                row_ptr[2*j*m0_+m0_+l] = -val;
            }
        }
    }

    // 3. 执行内存转置 (Transposition)
    // 这一步是性能起飞的关键：将 [vecsize][num_cols] 转为 [num_cols][vecsize]
    std::vector<float> projVal_transposed(static_cast<size_t>(vecsize_) * num_cols);
    #pragma omp parallel for schedule(static)
    for (int j = 0; j < num_cols; j++) {
        for (int i = 0; i < vecsize_; i++) {
            projVal_transposed[static_cast<size_t>(j) * vecsize_ + i] = 
                projVal_flat[static_cast<size_t>(i) * num_cols + j];
        }
    }

    const int large_m = 4 * m0_ * m0_;
    std::atomic<int> completed_tasks(0);
    int progress_step = std::max(1, large_m / 100);

    // 4. 并行排序段：现在内层循环是连续内存访问了
    #pragma omp parallel
    {
        // 线程私有 buffer，避免在循环内反复分配内存
        std::vector<Node> sorted_array(vecsize_);

        #pragma omp for schedule(dynamic, 4) // 使用 dynamic 应对负载不均
        for (int j = 0; j < large_m; j++) {
            int j1 = j / (2 * m0_);
            int j2 = j % (2 * m0_) + (2 * m0_); // 对应原逻辑的偏移映射

            float* col1 = projVal_transposed.data() + static_cast<size_t>(j1) * vecsize_;
            float* col2 = projVal_transposed.data() + static_cast<size_t>(j2) * vecsize_;

            // 这里的循环现在是连续读取，速度极快
            for (int l = 0; l < vecsize_; l++) {
                sorted_array[l].value = col1[l] + col2[l];
                sorted_array[l].index = mapTable[l];
            }
            ...
            // 批量拷贝到输出表
            Node* out_dest = InitialTable.data() + static_cast<size_t>(j) * maxk_;
            std::memcpy(out_dest, sorted_array.data(), maxk_ * sizeof(Node));

            // 降低原子操作频率，每 100 个任务汇报一次进度
            if (++completed_tasks % progress_step == 0) {
                #pragma omp critical
                {
                    std::cout << "\rProgress: " << (completed_tasks * 100 / large_m) << "%" << std::flush;
                }
            }
        }
    }
...
void buildTable(std::vector<std::vector<float>>& projVal_,
                std::vector<std::vector<float>>& vec0_,
                std::vector<Node>& InitialTable) {
    const int signed_m = 2 * m0_;
    const int pair_cols = signed_m * signed_m;                  // (2*m0_)^2
    const size_t table_cols = static_cast<size_t>(pair_cols) * pair_cols; // (2*m0_)^4
    const int subdim = extended_dim_ / 4;

    //if (extended_dim_ % 4 != 0) {
    //    throw std::runtime_error("extended_dim_ must be divisible by 4");
    //}

    // 每列最终保留 maxk_ 个点
    InitialTable.resize(table_cols * static_cast<size_t>(maxk_));
    //proj_flags.resize(table_cols);

    // ------------------------------------------------------------
    // 1) permutation0_ 重排整个数据集（沿用老代码逻辑，直接改 vec0_）
    // ------------------------------------------------------------
#pragma omp parallel
    {
        std::vector<float> tmp(extended_dim_);
#pragma omp for schedule(static)
    ...
    // ------------------------------------------------------------
    // 2) 计算 4 个子空间的 signed projection
    //
    // 存成转置形式：
    // projValT[(subspace * signed_m + sid) * vecsize_ + i]
    //
    // 若你还想保留旧 projVal_ 作为外部可见缓存，也一并写进去
    // projVal_[i][subspace * signed_m + sid]
    // ------------------------------------------------------------
    projVal_.assign(vecsize_, std::vector<float>(4 * signed_m, 0.0f));
    std::vector<float> projValT(static_cast<size_t>(4) * signed_m * vecsize_);

#pragma omp parallel for schedule(static)
    for (int i = 0; i < vecsize_; ++i) {
        const float* x = vec0_[i].data();

        for (int s = 0; s < 4; ++s) {
            const float* subvec = x + static_cast<size_t>(s) * subdim;

            for (int l = 0; l < m0_; ++l) {
                const float val = fstsubfunc0_(
                    (void*)subvec,
                    (void*)projVec0_[s][l].data(),
                    sub_func_param0_);

                const int pos_pos = s * signed_m + l;
                const int pos_neg = s * signed_m + (m0_ + l);

                projVal_[i][pos_pos] = val;
                projVal_[i][pos_neg] = -val;

                projValT[static_cast<size_t>(pos_pos) * vecsize_ + i] = val;
                projValT[static_cast<size_t>(pos_neg) * vecsize_ + i] = -val;
            }
        }
    }
    ...
    // 3) 预计算两个 pair 空间
    //
    // pair01T[(a,b), i] = proj(0,a) + proj(1,b)
    // pair23T[(c,d), i] = proj(2,c) + proj(3,d)
    // ------------------------------------------------------------
    std::vector<float> pair01T(static_cast<size_t>(pair_cols) * vecsize_);
    std::vector<float> pair23T(static_cast<size_t>(pair_cols) * vecsize_);

#pragma omp parallel for collapse(2) schedule(dynamic)
    for (int a = 0; a < signed_m; ++a) {
        for (int b = 0; b < signed_m; ++b) {
            const size_t pid = static_cast<size_t>(a) * signed_m + b;

            const float* col0 = projValT.data() + (static_cast<size_t>(0) * signed_m + a) * vecsize_;
            const float* col1 = projValT.data() + (static_cast<size_t>(1) * signed_m + b) * vecsize_;
            float* out = pair01T.data() + pid * vecsize_;
...
#pragma omp parallel for collapse(2) schedule(dynamic)
    for (int c = 0; c < signed_m; ++c) {
        for (int d = 0; d < signed_m; ++d) {
            const size_t pid = static_cast<size_t>(c) * signed_m + d;

            const float* col2 = projValT.data() + (static_cast<size_t>(2) * signed_m + c) * vecsize_;
            const float* col3 = projValT.data() + (static_cast<size_t>(3) * signed_m + d) * vecsize_;
            float* out = pair23T.data() + pid * vecsize_;
    ...
    // ------------------------------------------------------------
    // 4) 对整个数据集全排序（保持老代码风格）
    //    然后每列取前 maxk_ 放进 InitialTable
    // ------------------------------------------------------------
    std::atomic<size_t> completed_tasks(0);
    const size_t total_tasks = table_cols;
    const size_t progress_step = std::max<size_t>(1, total_tasks / 100);

#pragma omp parallel
    {
        std::vector<Node> buf(vecsize_);
...
#pragma omp for collapse(2) schedule(dynamic)
        for (int p01 = 0; p01 < pair_cols; ++p01) {
            for (int p23 = 0; p23 < pair_cols; ++p23) {
                ...
                const float* left  = pair01T.data() + static_cast<size_t>(p01) * vecsize_;
                const float* right = pair23T.data() + static_cast<size_t>(p23) * vecsize_;

                for (int i = 0; i < vecsize_; ++i) {
                    buf[i].value = left[i] + right[i];
                    buf[i].index = mapTable[i];   // 保持老代码行为
                }
                ...
                Node* out = InitialTable.data() + col_id * static_cast<size_t>(maxk_);
                std::memcpy(out, buf.data(), static_cast<size_t>(maxk_) * sizeof(Node));
                const size_t done = ++completed_tasks;
...
#pragma omp critical
    ...
    std::cout << "\rProgress: 100%!" << std::endl;
        ...
        void compression(int vecsize){
            ...
            for(int i = 0; i < vecsize; i++){
                mapTable[i] = convert_id2[mapTable[i]];
			} 			
