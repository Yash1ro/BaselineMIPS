...
void FastGraph(int efc_, int M_, int data_size_, int query_size_, int dim_, char* path_q_, char* path_data_, char* truth_data_, char* path_index_, int L_, int topk_) {
    ...
    size_t vecdim = dim_;
	size_t padded_dim = (vecdim + 15) & ~0xF;   //warning
    size_t extended_dim = ((padded_dim + L_ - 1) / L_) * L_;   //warning
    ...
    std::vector<std::vector<std::vector<float>>> projVec0(
        4, std::vector<std::vector<float>>( 
            m0, std::vector<float>(extended_dim/4, 0.0f))); 

    std::vector<Node> InitialTable;
    InitialTable.resize(static_cast<size_t>(16*m0*m0*m0*m0) * maxk);
...
//--------------------------------------------------
    ...
    if (fs::exists(dir)) {
        ...
        for(int i = 0; i < vecsize; i++){
            ...
            float sum = 0;
            for(int j = 0; j < vecdim; j++){
                //float tmp = buf[j] - center_data[j];
                float tmp = buf[j];
				sum += tmp * tmp;
            }
    
            //array[i].first = sum;
            array[i].first = -1.0f * sum;			
            array[i].second = i;
        }
        ...
        for(int i = 0; i < step; i++) {
            ...
            std::streampos absolute_offset = static_cast<long long>(target_id) * (static_cast<long long>(vecdim) * 4);
        ...
        std::vector<std::vector<float>> projVal(vecsize, std::vector<float>(8 * m0, 0.0f));
        ...
        appr_alg = new HierarchicalNSW<float>(maxk, step, m, vecdim, projVec, level, subdim, permutation, &l2space, &ipspace, &ipsubspace, &ipsubspace0, vecsize, 
		    path_index, m0, projVec0, permutation0, M, efConstruction);

        //appr_alg->allocateGraphIndex(projVal, vec0);        
        //appr_alg->buildTable(projVal, vec0);
        ...
        for (int i = 0; i < step; i++) {
			...
			int cur_id = sorted_targets[i].second;
            appr_alg->addPoint((void *) mass, (size_t) cur_id, &(true_norm[cur_id]));
            delete[] mass;
		}			
...
for (int i = 0; i < vecsize; i++) {
    ...
    std::streampos absolute_offset = static_cast<long long>(i) * (static_cast<long long>(vecdim) * 4);
    ...
    appr_alg->addPoint((void *) mass, (size_t) i, &(true_norm[i]));
    delete[] mass;
}		
...
//--------------------------
        std::vector<std::vector<float>> vec0(
            vecsize, std::vector<float>(extended_dim, 0.0f));
		...
		for(int i = 0; i < vecsize; i++){
            input.read((char*)buf.data(), sizeof(float) * vecdim);
            memcpy(vec0[i].data(), buf.data(), sizeof(float) * vecdim);
            memset(vec0[i].data() + vecdim, 0, sizeof(float) * (extended_dim - vecdim));							
		}

        input.close();
		
		appr_alg->buildTable(projVal, vec0, InitialTable);
...
void FastGraph(int efc_, int M_, int data_size_, int query_size_, int dim_,
    ...
    //size_t maxk = 100;
	size_t maxk = topk_;
    size_t gt_maxk = 100; // truth file always has 100 neighbors per query
    size_t vecsize = data_size_;
    size_t qsize = query_size_;
    size_t vecdim = dim_;
    size_t padded_dim = (vecdim + 15) & ~0xF;
    ...
    // ------------------------------------------------------------
    // 1) 一次性顺序读入所有原始数据，保留“原始 id -> 向量”的对应关系
    //    raw_data[i * extended_dim ...] 对应原始第 i 个点
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // 2) 前 step 个 seed，按原始 id 顺序取出
    //    result_vectors[i] 对应原始 id = i
    // ------------------------------------------------------------
    std::vector<std::vector<float>> result_vectors(step, std::vector<float>(vecdim, 0.0f));
    for (int i = 0; i < step; i++) {
    ...
    // ------------------------------------------------------------
    // 3) init_points 按 step 个种子构造；顺序与原始 id 一致
    // ------------------------------------------------------------
    float** init_points = new float*[step];
    for (int i = 0; i < step; i++) {
    ...
    // ------------------------------------------------------------
    // 4) 前 step 个点按原始 id 顺序插入
    // ------------------------------------------------------------
    for (int i = 0; i < step; i++) {
        const int cur_id = i;
        appr_alg->addPoint((void*)init_points[i], (size_t)cur_id, &(true_norm[cur_id]));
    }
...
#pragma omp parallel for schedule(dynamic)
    for (int ord = step; ord < (int)vecsize; ord++) {
        const int cur_id = ord;
...
#pragma omp critical(progress_print)
        {
            progress_count++;
            if (progress_count % report_every == 0) {
                cout << (step + progress_count) / (0.01 * vecsize) << " %, "
                     << report_every / (1000.0 * 1e-6 * stopw.getElapsedTimeMicro()) << " kips\n";
                stopw.reset();
            }
        }
...
#pragma omp parallel for schedule(dynamic)
...

    {
        std::ofstream fout0(fullPath, std::ios::binary);
        if (!fout0) {
            for (int i = 0; i < step; i++) delete[] init_points[i];
            delete[] init_points;
