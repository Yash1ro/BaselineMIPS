#!/usr/bin/env python3
"""PAG_without_projection benchmark sweep (identical runner logic to benchmark_pag.py)."""
...
def run(config: DatasetConfig):
    """Run PAG_without_projection benchmark script and parse recall-QPS lines."""
    pag_dir = Path(config.pag_without_proj_dir)
    script_name = config.pag_without_proj_run_script if config.pag_without_proj_run_script else f"run_{config.name}.sh"
    script_path = pag_dir / script_name
    if not script_path.exists():
        print(f"[PAG_noproj] script not found: {script_path}")
        return [], 0.0, 0.0, 0.0

    # Derive index directory from iPath line in the script
    index_dir = pag_dir / f"index_noproj_{config.name}"
    try:
    ...
    for i in range(run_times):
        print(f"[PAG_noproj] run {i + 1}/{run_times}: {script_name}")
        if run_times == 2 and i == 0:
            ...
            print(f"[PAG_noproj] index build time: {build_time:.2f}s, peak mem: {build_peak_mb:.1f} MB")
        else:
            env = os.environ.copy()
    ...
    if not points:
        print("[PAG_noproj] no parsable points from output")
    else:
        print(f"[PAG_noproj] parsed {len(points)} points")
